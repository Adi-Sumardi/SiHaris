...
@section('content')
    <div x-data="employeeExitLiveFilter()">
        {{-- Stats Cards --}}
            ...
            <div class="card-body-sm">
                <form id="employee-exits-filter-form" action="{{ route('employee-exits.index') }}" method="GET" class="flex flex-wrap items-end gap-3" @submit.prevent="fetchData()">
                    ...
                    <div class="w-40">
                        ...
                        <select name="exit_type" id="exit_type" class="input w-full" @change="fetchData()">
                            <option value="">Semua</option>
                    ...
                    <div class="flex items-center gap-2">
                        <button type="button" @click="resetFilters()" class="btn btn-ghost btn-sm" id="reset-btn" style="{{ request()->hasAny(['status', 'exit_type', 'search']) ? '' : 'display: none;' }}">
                            Reset
                        </button>
        ...
        {{-- Exit List --}}
        <div class="card" id="employee-exits-card">
            <x-table>
                <x-slot name="header">
                    <th>Karyawan</th>
                    <th>Jenis Exit</th>
                ...
                @forelse($employeeExits as $exit)
                    <tr class="hover:bg-secondary-50/60 transition-colors">
                        <td>
                            <div class="flex items-center gap-2.5">
                                <div class="w-8 h-8 rounded-full bg-primary-100 flex items-center justify-center flex-shrink-0">
                                    <span class="text-primary-700 text-xs font-medium">{{ strtoupper(substr($exit->employee->full_name ?? 'K', 0, 1)) }}</span>
                                </div>
                                <div class="min-w-0">
                                    <span class="font-medium text-secondary-900 block truncate">{{ $exit->employee->full_name }}</span>
                                    <p class="text-xs text-secondary-400 font-mono">{{ $exit->employee->employee_id }}</p>
                                </div>
                            </div>
                        </td>
                        <td>
                        ...
                        <td class="text-secondary-700 text-xs">{{ $exit->effective_date->format('d M Y') }}</td>
                        <td>
                        ...
                        <td>
                            <div class="flex items-center gap-2">
                                ...
                                <span class="text-xs text-secondary-500 font-mono">{{ $exit->checklist_progress }}%</span>
                            </div>
                        </td>
                        <td>
                    ...
                    <tr>
                        <td colspan="6" class="text-center py-12">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-secondary-500">Belum ada data exit karyawan yang sesuai dengan filter.</p>
    ...
    <script>
        function employeeExitLiveFilter() {
            return {
                loading: false,
                debounceTimer: null,
                ...
                updateResetVisibility() {
                    ...
                    const exitTypeVal = document.getElementById('exit_type')?.value || '';
                    const resetBtn = document.getElementById('reset-btn');
                    if (resetBtn) {
                        resetBtn.style.display = (searchVal || statusVal || exitTypeVal) ? '' : 'none';
                    }
                },
                ...
                resetFilters() {
                    ...
                    const exitTypeInput = document.getElementById('exit_type');

                    if (searchInput) searchInput.value = '';
                    if (statusInput) statusInput.value = '';
                    if (exitTypeInput) exitTypeInput.value = '';
                ...
                fetchData(isReset = false) {
                    const form = document.getElementById('employee-exits-filter-form');
                    if (!form) return;
                    ...
                    .then(html => {
                        ...
                        const newCard = doc.getElementById('employee-exits-card');
                        const currentCard = document.getElementById('employee-exits-card');
