...
@section('title', 'Edit Exit Karyawan')
...
@section('breadcrumb')
    ...
    <span class="text-slate-700 font-medium">Edit Exit</span>
@endsection

@section('header')
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            <h1 class="text-2xl font-bold text-secondary-900">Edit Exit Karyawan</h1>
            <p class="text-secondary-500 mt-1">{{ $employeeExit->employee->full_name }}</p>
        </div>
        <div>
            <a href="{{ route('employee-exits.show', $employeeExit) }}" class="btn btn-ghost">
...
@section('content')
    <form action="{{ route('employee-exits.update', $employeeExit) }}" method="POST" class="w-full">
        @csrf
        @method('PUT')
        ...
        <div class="card">
            ...
            <div class="card-body space-y-4">
                ...
                <div>
                    ...
                    <select name="employee_id" id="employee_id" class="input w-full @error('employee_id') border-danger-500 @enderror" required>
                        ...
                        @foreach($employees as $employee)
                            <option value="{{ $employee->id }}" {{ old('employee_id', $employeeExit->employee_id) == $employee->id ? 'selected' : '' }}>
                ...
                <div>
                    ...
                    <select name="exit_type" id="exit_type" class="input w-full @error('exit_type') border-danger-500 @enderror" required>
                        ...
                        <option value="resignation" {{ old('exit_type', $employeeExit->exit_type) === 'resignation' ? 'selected' : '' }}>Resign (Pengunduran Diri)</option>
                        <option value="termination" {{ old('exit_type', $employeeExit->exit_type) === 'termination' ? 'selected' : '' }}>Terminasi (PHK)</option>
                        <option value="retirement" {{ old('exit_type', $employeeExit->exit_type) === 'retirement' ? 'selected' : '' }}>Pensiun</option>
                        <option value="contract_end" {{ old('exit_type', $employeeExit->exit_type) === 'contract_end' ? 'selected' : '' }}>Kontrak Berakhir</option>
                        <option value="mutual_agreement" {{ old('exit_type', $employeeExit->exit_type) === 'mutual_agreement' ? 'selected' : '' }}>Kesepakatan Bersama</option>
                        <option value="death" {{ old('exit_type', $employeeExit->exit_type) === 'death' ? 'selected' : '' }}>Meninggal Dunia</option>
                    </select>
                    @error('exit_type')
                        <p class="mt-1 text-sm text-danger-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <input type="date" name="request_date" id="request_date"
                           value="{{ old('request_date', $employeeExit->request_date->format('Y-m-d')) }}"
                ...
                <div>
                    ...
                    <input type="date" name="last_working_day" id="last_working_day"
                           value="{{ old('last_working_day', $employeeExit->last_working_day->format('Y-m-d')) }}"
                ...
                <div>
                    ...
                    <input type="date" name="effective_date" id="effective_date"
                           value="{{ old('effective_date', $employeeExit->effective_date->format('Y-m-d')) }}"
                ...
                <div>
                    ...
                    <textarea name="reason" id="reason" rows="3"
                              ...
                              placeholder="Jelaskan alasan exit...">{{ old('reason', $employeeExit->reason) }}</textarea>
                    @error('reason')
                        <p class="mt-1 text-sm text-danger-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    ...
                    <textarea name="notes" id="notes" rows="2"
                              ...
                              placeholder="Catatan tambahan (hanya untuk internal)...">{{ old('notes', $employeeExit->notes) }}</textarea>
                    @error('notes')
                        <p class="mt-1 text-sm text-danger-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="border-t pt-4">
                    <label class="flex items-center gap-3">
                        <input type="checkbox" name="is_rehire_eligible" value="1"
                               ...
                               {{ old('is_rehire_eligible', $employeeExit->is_rehire_eligible) ? 'checked' : '' }}>
                ...
                <div>
                    ...
                    <textarea name="rehire_notes" id="rehire_notes" rows="2"
                              ...
                              placeholder="Catatan terkait eligibilitas rehire...">{{ old('rehire_notes', $employeeExit->rehire_notes) }}</textarea>
                    @error('rehire_notes')
                        <p class="mt-1 text-sm text-danger-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="card-footer flex justify-end gap-3">
                <a href="{{ route('employee-exits.show', $employeeExit) }}" class="btn btn-ghost">Batal</a>
