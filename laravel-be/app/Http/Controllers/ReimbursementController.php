...
class ReimbursementController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse
    {
        ...
        $category = ReimbursementCategory::where('company_id', $tenant->id)
            ->find($request->category_id);

        $rules = [
            'employee_id' => ['required', Rule::exists('employees', 'id')->where('company_id', $tenant->id)],
            'category_id' => ['required', Rule::exists('reimbursement_categories', 'id')->where('company_id', $tenant->id)],
            'amount' => [
                'required',
                'numeric',
                'min:1000',
        ...
        try {
            DB::transaction(function () use ($tenant, $validated, $receiptPath) {
    ...
    public function approve(Request $request, Reimbursement $reimbursement): RedirectResponse
    {
        ...
        if ($reimbursement->hasWorkflow()) {
            $result = app(ApprovalService::class)->processApproval(
                $reimbursement, auth()->user(), 'approve', $request->notes
            );
    ...
    public function reject(Request $request, Reimbursement $reimbursement): RedirectResponse
    {
        ...
        if ($reimbursement->hasWorkflow()) {
            $result = app(ApprovalService::class)->processApproval(
                $reimbursement, auth()->user(), 'reject', $validated['rejection_reason']
            );
        ...
        // Legacy: no workflow
        $reimbursement->reject(auth()->id(), $validated['rejection_reason']);
