...
@section('title', 'Tambah Mesin Fingerprint')
...
@section('header')
    <h1 class="text-2xl font-bold text-secondary-900">Tambah Mesin Fingerprint</h1>
@endsection

@section('content')
    <div class="card max-w-2xl">
        <form method="POST" action="{{ route('fingerprint-devices.store') }}">
            @csrf
            <div class="card-body space-y-4">
                <div>
                    ...
                    <input type="text" name="name" id="name" value="{{ old('name') }}" class="input w-full @error('name') border-danger-500 @enderror" placeholder="Mesin Lobby Lt.1" required>
                ...
                <div>
                    ...
                    <select name="brand" id="brand" class="input w-full @error('brand') border-danger-500 @enderror" required>
                        ...
                        <option value="fingerspot" {{ old('brand') === 'fingerspot' ? 'selected' : '' }}>Fingerspot</option>
                        <option value="solution" {{ old('brand') === 'solution' ? 'selected' : '' }}>Solution (X100C, dll)</option>
                ...
                <div>
                    ...
                    <input type="text" name="serial_number" id="serial_number" value="{{ old('serial_number') }}" class="input w-full font-mono @error('serial_number') border-danger-500 @enderror" required>
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="ip_address" id="ip_address" value="{{ old('ip_address') }}" class="input w-full @error('ip_address') border-danger-500 @enderror" placeholder="192.168.1.201">
                    ...
                    <div>
                        ...
                        <input type="number" name="port" id="port" value="{{ old('port', 4370) }}" class="input w-full @error('port') border-danger-500 @enderror">
                        @error('port')<p class="mt-1 text-sm text-danger-600">{{ $message }}</p>@enderror
                    </div>
                </div>

                <label class="flex items-center gap-2">
                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="rounded border-secondary-300">
