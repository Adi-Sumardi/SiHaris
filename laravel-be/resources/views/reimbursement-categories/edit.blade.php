...
@section('title', 'Edit Kategori Reimbursement')
...
@section('content')
    <div class="card w-full">
        ...
        <form action="{{ route('reimbursement-categories.update', $category) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="card-body space-y-4">
                ...
                <div>
                    ...
                    <textarea name="description" id="description" rows="3"
                              class="input w-full @error('description') border-danger-500 @enderror">{{ old('description', $category->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-danger-600">{{ $message }}</p>
                    @enderror
                </div>

                <div x-data="currencyInput({{ old('max_amount', $category->max_amount ?? 0) }})">
                ...
                <div class="space-y-3">
                    <label class="flex items-center gap-3 cursor-pointer">
                        ...
                        <input type="checkbox" name="requires_receipt" value="1"
                               {{ old('requires_receipt', $category->requires_receipt) ? 'checked' : '' }}
